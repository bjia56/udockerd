...
**`RUN` steps dying with SIGSYS (signal 31) under `docker build`**: udocker's `PRootEngine` defaults to proot's ptrace-based seccomp acceleration ("seccomp mode 2") unless `PROOT_NO_SECCOMP` is set — checked via `os.getenv("PROOT_NO_SECCOMP")` directly in `engine/proot.py`, not through a `Config.conf` key, so it can't be forced via `configure_udocker`'s usual `UDOCKER_*` env-override table. Android/Termux kernels frequently can't correctly emulate syscalls under that acceleration path; the kernel responds with SIGSYS and proot kills the traced process mid-syscall — reproduces reliably on any nontrivial `RUN` (e.g. `apt-get install`), not specific to that command. `configure_udocker()` sets `PROOT_NO_SECCOMP=1` (via `os.environ.setdefault`, same override-friendly pattern as the curl var) to force proot onto its slower non-accelerated ptrace path everywhere, since the daemon only ever targets Termux/Android.
...
**Preferring Termux's own `proot` package over udocker's bundled one**: `config.py`'s `is_termux()` (checks `TERMUX_VERSION`/`PREFIX`) gates two things. First, `configure_udocker()` sets `UDOCKER_USE_PROOT_EXECUTABLE` (udocker's own env-override, read by `Config().getconf()`) to `shutil.which("proot")` when found, in favor of udocker's bundled binary — proot-distro deployments (the guest the docker CLI runs from) already depend on Termux's `proot` package to create/enter distro rootfs, so it's present with zero extra install in practice, and Termux's build is maintained specifically for Android SELinux/seccomp quirks. Flag differences from what udocker's bundle would have are already probed dynamically (`HostInfo.cmd_has_option`) everywhere they matter, so this degrades gracefully rather than breaking if Termux's build lags a flag. Second, `udocker_ctx.py`'s `_patched_untar_layers` skips the bare-tar attempt entirely on Termux once a `--link2symlink`-capable proot is confirmed (`_capable_proot_executable`) — the first attempt is known-doomed there (hardlink() always rejected), so trying it first only wastes a full extraction pass before falling back. Hosts where no capable proot is found still get the real bare-tar attempt, so this can't turn a working extraction into a false failure.
...
**DNS (`/etc/resolv.conf`).** udocker's default `sysdirs_list` bind (`engine/base.py`'s `_set_volume_bindings`) tries to bind the *host's* `/etc/resolv.conf` over the container's, same idea as dockerd's own resolv.conf bind-mount — but Termux/Android ships no such host file at all, and `LocalRepository`'s volume check silently drops any sysdirs binding whose host path doesn't exist, so that bind is always a no-op here. Whether a container ends up with working DNS then depends entirely on what its image happened to bake into `/etc/resolv.conf`: Debian ships one with real nameservers, Ubuntu ships an intentionally-empty placeholder expecting the runtime to supply one — so `apt update` works in `debian` but silently fails to resolve anything in `ubuntu`.

Fix: `udocker_ctx.new_resolv_conf()` generates a resolv.conf (nameservers configurable via `--dns`/`UDOCKERD_DNS`, default `8.8.8.8`/`8.8.4.4`) and `container_proc.apply_default_opt()` binds it explicitly onto every container's `/etc/resolv.conf`, overriding whatever the image shipped. One fresh file per container run, not a single shared path: proot's `-b` bind has no read-only mode, so a container that rewrites `/etc/resolv.conf` (e.g. `resolvconf`/`systemd-resolved` tooling) would otherwise corrupt a shared file for every other container — mirrors udocker's own per-run tmp passwd/group bind files.
