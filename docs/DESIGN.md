...
- **Every long-lived streaming response needs `Connection: close`.** With `protocol_version = "HTTP/1.1"` (needed for hijack, above), the stdlib default is keep-alive. `/wait`, `/logs?follow`, `/attach`, and exec's non-hijack fallback all have unknown body length up front and no natural end-of-body marker other than the connection closing — without this header the server tries to keep the connection open for a next request while the client waits for close as its end-of-stream signal, and neither side ever terminates. `docker logs -f` against an already-exited container hung indefinitely without this.
