...
- **Every long-lived streaming response needs `Connection: close`.** With `protocol_version = "HTTP/1.1"` (needed for hijack, above), the stdlib default is keep-alive. `/wait`, `/images/create`, `/logs?follow`, `/attach`, and exec's non-hijack fallback all have unknown body length up front and no natural end-of-body marker other than the connection closing — without this header the server tries to keep the connection open for a next request while the client waits for close as its end-of-stream signal, and neither side ever terminates. `docker logs -f` against an already-exited container hung indefinitely without this — as did the Python SDK's `images.pull()`, which manual `curl` testing never caught because curl doesn't care about clean stream termination the way a stricter client does.
- **Hijacked connections need to be force-closed server-side once the handler returns.** `101 UPGRADED` correctly uses `Connection: Upgrade`, not `close` — the connection is meant to become raw, not close immediately. But `BaseHTTPRequestHandler`'s request loop doesn't know that: once the hijack handler function returns, it tries to parse a *next* HTTP request off what is now, from the client's perspective, a finished and abandoned socket. The client (e.g. the SDK's default blocking `exec_run`, which reads hijacked output until EOF) hangs waiting for the connection to close, while the server waits for a request line that will never arrive. Fix: set `self.close_connection = True` on the handler right after sending the `101` response, so the server's own request loop closes the socket once the hijack handler returns.

### Interactive sessions (TTY)
`docker exec -it` / `docker run -it` need a real pty, not just piped stdio, for correct raw-mode passthrough (no multiplex framing) and terminal semantics.

- **Ctty assignment across the supervisor/fork boundary**: the process opening a pty slave must be a session leader with no controlling terminal at that moment. The supervisor (see above) already calls its own `setsid()` for PDEATHSIG/pgid purposes, so its forked child — which execs the real command — is a session *member*, not leader, and can't just open the slave. Fixed in `supervisor.c` by having the **forked child** call `setsid()` again itself (valid: it's not a process-group leader) immediately before opening the slave and dup2'ing it onto its stdin/stdout/stderr. This necessarily puts the child in a different session/pgid than the supervisor — a pgid can't span two sessions — so supervisor's crash/shutdown cleanup does `killpg(child_pid, SIGKILL)` in addition to its own `killpg(0, ...)` in TTY mode, and does so in that order (killing its own group first would kill itself before the second call ever runs).
- **Output fan-out**: a pty master's bytes are destructive to read (no broadcast), so a single dedicated reader thread per TTY session (`spawn_tty_reader`) owns the master fd, writes every chunk to the logfile (so `docker logs` works even with no live client attached) and fans out to a small subscriber list of currently-attached exec/attach connections. `docker exec`/`attach` subscribe for the connection's lifetime and unsubscribe on disconnect or process exit; disconnecting doesn't kill the process (matches real Docker — a session persists after detach unless the container/exec is explicitly stopped).
- **Stdin forwarding**: a per-connection thread reads the hijacked connection's `rfile` byte-at-a-time and writes to the pty master, for as long as the client stays attached.
- **Non-TTY output stays multiplex-framed** as before; the TTY path is raw passthrough. `stream_session()` in `container_proc.py` dispatches between the two based on `ContainerProc.tty`.
